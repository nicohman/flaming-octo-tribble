...
requirejs(['./js/var.js'], function(vars, lib){
var rightDown;
var leftDown;
var spaceDown;
var Tank = {
...
window.onload = function() {
  ...
  window.drawImage = function(img, posX, posY) {
  ...
  window.bull = imgLoad('./img/bullet.png');
  window.bomb = imgLoad('./img/dabomb.png');
  var grass = imgLoad('./img/g.png');
  window.splasher = imgLoad('./img/splash.png');
  window.blimpie = imgLoad('./img/blimpie.png', function() {});
  window.frame1 = imgLoad('./img/frame1.png', function() {});
  window.frame2 = imgLoad('./img/frame2.png', function() {});
  window.frame3 = imgLoad('./img/frame3.png', function() {});
  ...
  $('#body').keydown(function(event) {
    ...
    var animate = function() {
      ...
      if (vars.coolDownL1 < 1) {
        ...
        vars.coolDownL1 = 66.66;
      }
      if (vars.coolDownL0 < 1) {
        var wilb;
      ...
      if (vars.coolDownL2 < 1) {
        ...
        vars.coolDownL2 = 166.66;
      }
      //Move the planes
      planes.forEach(function(val, index, arr) {
        var imig;
        if (arr[index].layer === 2) {
          planes[index].topX = planes[index].topX + (level * 0.1) + (vars.blimpSpeed * planes[index].orientL);
          imig = window.planeImgs['B'];
        } else if (val.layer == 0 && val.orientL == -1) {
          planes[index].topX = planes[index].topX + (level * 0.1) + (vars.planeSpeed * planes[index].orientL);
          imig = window.planeImgs['L'];
        } else {
          planes[index].topX = planes[index].topX + (level * 0.1) + (vars.planeSpeed * planes[index].orientL);
          imig = window.planeImgs['R'];
        }
      ...
      vars.coolDownL1 = vars.coolDownL1 - 1;
      vars.coolDownL0 = vars.coolDownL0 - 1;
      vars.coolDownL2 = vars.coolDownL2 - 1;
      //Collison detection/audio playing.

      bombs.forEach(function(val, index, arr) {
        drawImage(window.bomb, val.topX, val.topY);
    ...
    if (window.requestAnimationFrame && gameOver !== true) {
      ...
      intervalID = setInterval(animate, vars.frameRate);
    }
  })
